...
#   - For each drive, prompts user to type YES to confirm wiping
#   - Unmounts any mounted partitions on the device
#   - Runs wipefs to clear filesystem signatures
#   - Runs shred with 3 random passes and a final zero pass
#
# Notes:
#   - Targets only removable drives (RM=1)
#   - Will not touch internal disks (NVMe, SATA, etc.)
#   - Requires user to confirm each device wipe
################################################################################
...
for cmd in lsblk umount wipefs shred; do
    if ! command -v $cmd &> /dev/null; then
        ...
        pacman -Sy --noconfirm $cmd || { echo "Failed to install $cmd. Exiting."; exit 1; }
    fi
done

echo "=== Secure wipe of all external storage devices ==="

# Detect removable drives; strip partition numbers; get unique device paths
USB_DRIVES=$(lsblk -o NAME,RM -nr | awk '$2 == 1 { print "/dev/" $1 }' | sed 's/[0-9]*$//' | sort -u)

# Exit if no external drives detected
